...
impl Ssd1306Ui {
    pub fn new() -> Self {
        Self { display: Ssd1306::new(SSD1306_ADDR) }
    }
    ...
    fn draw_header(&mut self, app: &AppState) {
        ...
        self.display.draw_str(COL_ENABLE, ROW_HEADER,
            if app.supply.enabled { "ON" } else { "OFF" });

        // Separator line marks the hardware colour boundary
        self.display.draw_line(0, ROW_DIVIDER, DISPLAY_W - 1, ROW_DIVIDER);
    }
    ...
    fn draw_telemetry(&mut self, app: &AppState) {
        draw_row(&mut self.display, ROW_VIN,  "Vin ",
            app.telemetry.vin_mv,  Unit::Voltage, VIN_RANGE);
        draw_row(&mut self.display, ROW_VOUT, "Vout",
            app.telemetry.vout_mv, Unit::Voltage, VOUT_RANGE);
        draw_row(&mut self.display, ROW_IOUT, "Iout",
            app.telemetry.iout_ma, Unit::Current, IOUT_RANGE);
        draw_row(&mut self.display, ROW_POUT, "Pout",
            app.telemetry.pout_mw, Unit::Power,   POUT_RANGE);
    }

    /// Thin bar showing output power relative to the configured v_set × i_set ceiling.
    fn draw_power_bar(&mut self, app: &AppState) {
        let max_mw = ((app.supply.v_set_mv as u64 * app.supply.i_set_ma as u64) / 1_000)
            .max(1);
        let bar_len = ((app.telemetry.pout_mw as u64 * DISPLAY_W as u64) / max_mw)
            .min(DISPLAY_W as u64) as u8;

        if bar_len > 0 {
            self.display.draw_line(0, ROW_POWER_BAR, bar_len - 1, ROW_POWER_BAR);
        }
    }
...
#[derive(Clone, Copy)]
enum Unit { Voltage, Current, Power }
...
fn draw_setpoint_right(d: &mut Ssd1306, millivalue: u32, unit: Unit) {
    ...
    d.draw_str(x + (PREFIX.len() + val.len()) as u8 * FONT_W, ROW_STATUS, sym);
}
